...
## 2026-03-19 - Optimize MediaQuery Selectors
**Learning:** Using `MediaQuery.of(context)` causes unnecessary widget rebuilds when unrelated properties like `viewInsets` (keyboard state) change. This is especially problematic in core widgets like `AppCachedImage` which are used heavily in list/grid views.
**Action:** Always use specific selectors like `MediaQuery.sizeOf(context)` or `MediaQuery.devicePixelRatioOf(context)` to limit rebuilds only to when those specific properties change.
