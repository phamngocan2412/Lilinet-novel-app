...
## 2026-03-15 - Expensive Set Derivation in Build Method
**Learning:** Re-calculating derived sets (e.g., mapping and sorting a list, an O(N log N) operation) directly inside a `BlocBuilder` build method executes on every widget rebuild (such as scrolling or window resizing), severely impacting performance.
**Action:** Always pre-compute and memoize derived states (like sets or maps from lists) in the Bloc so they are only recalculated when the underlying data changes, and emit them as part of the state.
