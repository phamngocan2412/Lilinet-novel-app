...
void main() {
  ...
  const tAppSettings = AppSettings(pinCode: '1234', themeMode: ThemeMode.dark);
      ...
      () async {
        ...
        verify(
          () => mockSharedPreferences.setString(
            'app_settings',
            any(
              that: predicate<String>((jsonString) {
  ...
  group('getSettings', () {
    test('should merge sensitive data from SecureStorage', () async {
      ...
      when(
        () => mockSharedPreferences.getString('app_settings'),
      ).thenReturn(jsonEncode(tAppSettingsJson));
      when(
        () => mockSecureStorage.read(key: 'settings_pin_code'),
    ...
    test(
      'should return settings with null pinCode if not in SecureStorage',
      () async {
        // arrange
        final tAppSettingsJson = tAppSettings.toJson();
        tAppSettingsJson.remove('pinCode');
